...
module.exports.onStart = async function ({event, api, args, usersData, __GLOBAL }) {
   ...
   if (color == 0 && check(random) == '💙') api.sendMessage(`✨ —- مبارك عليك 😍 -— ✨\nاختيارك موافق لاختياري: (💙)\n\n  خذ: ${coin} $ جائزة لفوزك 🤭`, event.threadID, event.messageID);await usersData.set(senderID, {money: userData.money + coinsmhmd1, data: userData.data}); 
   ...
   else api.sendMessage(`خطأ 🤣، اختياري هو: (${check(random)})\nحاول مجددا، وأتمنى تفوز 🥹\n            -----------------\n             - 1000 $`, event.threadID,event.messageID); await usersData.set(senderID, {money: userData.money + coinsmhmd1, data: userData.data});
       }
